  ...
  if (command === 'leaderboard') {
    ...
    if (message.mentions.users.first()) {
      ...
      eco.Leaderboard({
        ...
        if (users[0]) var firstplace = await client.filter()//Searches for the user object in discord for first place
        if (users[1]) var secondplace = await client.filter()//Searches for the user object in discord for second place
        if (users[2]) var thirdplace = await client.filter()//Searches for the user object in discord for third place
...
if(!args) return message.reply("add number please")
    await eco.AddToBalance(user, args)
  }
